...
public class MSDRDocument : IDocument
{
    ...
    public void Compose(IDocumentContainer container)
    {
        container.Page(page =>
        {
            ...
            page.Header().DefaultTextStyle(x => x.FontFamily("Liberation Subset")).Element(ComposeHeader);
            page.Content().DefaultTextStyle(x => x.FontFamily("Liberation Subset")).Element(ComposeContentFirstPage);
            page.Size(PageSizes.A4);
        });
        container.Page(page =>
        {
            page.Margin(40, Unit.Point);
            page.Header().DefaultTextStyle(x => x.FontFamily("Liberation Subset")).Element(ComposeHeader2);
            page.Content().DefaultTextStyle(x => x.FontFamily("Liberation Subset")).Element(ComposeContentSecondPage);
            page.Size(PageSizes.A4);
        });
    }
